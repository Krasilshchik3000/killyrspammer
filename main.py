...
ИТОГОВЫЙ_ПРОМПТ: [полный промпт с сохраненными старыми критериями + новыми дополнениями]"""
...
Ответь в формате:
АНАЛИЗ: [причина ошибки]
ИТОГОВЫЙ_ПРОМПТ: [полный промпт с сохраненными старыми критериями + новыми исключениями/уточнениями]"""
    ...
    try:
        ...
        if "ИТОГОВЫЙ_ПРОМПТ:" in analysis:
            ...
            # Проверяем, что промпт содержит необходимые элементы
            if "{message_text}" not in improved_prompt:
                logger.warning("⚠️ ChatGPT не включил {message_text}, добавляю принудительно")
