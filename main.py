...
ВАЖНО: НЕ меняй системные части промпта! Улучши только критерии в середине.

СИСТЕМНЫЕ ЧАСТИ (НЕ ТРОГАТЬ):
- Начало: "Проанализируй сообщение из телеграм-группы и ответь строго одним из трёх вариантов: СПАМ НЕ_СПАМ ВОЗМОЖНО_СПАМ"
- Конец: "Сообщение: «{{message_text}}» Ответ:"

Ответь в формате:
АНАЛИЗ: [причина ошибки]
УЛУЧШЕННЫЕ_КРИТЕРИИ: [только улучшенные критерии для середины промпта, без системных частей]"""
...
Почему ты был неуверен? Улучши критерии, чтобы такие сообщения определялись как СПАМ.

ВАЖНО: НЕ меняй системные части промпта! Улучши только критерии в середине.

СИСТЕМНЫЕ ЧАСТИ (НЕ ТРОГАТЬ):
- Начало: Проанализируй сообщение из телеграм-группы и ответь строго одним из трёх вариантов
- Конец: Сообщение и Ответ

Ответь в формате:
АНАЛИЗ: [почему был неуверен]
УЛУЧШЕННЫЕ_КРИТЕРИИ: [более четкие критерии для определения таких сообщений как СПАМ]"""
...
УЛУЧШЕННЫЕ_КРИТЕРИИ: [только улучшенные критерии для середины промпта, без системных частей]"""
    ...
    try:
        ...
        # Извлекаем улучшенные критерии и собираем полный промпт
