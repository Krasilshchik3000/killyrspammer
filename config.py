...
# Классификация: mini-тир предпочтительнее nano — задача нюансная
# (разговорный русский, замаскированный спам, OCR баннеров через Vision),
# а разница в цене при наших объёмах — центы в день.
LLM_MODEL_CANDIDATES = [
    "gpt-5.5-mini", "gpt-5.4-mini", "gpt-5-mini", "gpt-4.1-mini",
    "gpt-5.5-nano", "gpt-5.4-nano", "gpt-5-nano", "gpt-4o-mini",
]
...
# ВАЖНО: у reasoning-моделей (gpt-5.x) в max_completion_tokens входят
# невидимые reasoning-токены. 150 не хватало — модель «думала» на весь лимит,
# JSON-ответ оставался пустым → fallback → случайные ВОЗМОЖНО_СПАМ.
LLM_MAX_TOKENS = int(os.getenv("LLM_MAX_TOKENS", "600"))
LLM_TEMPERATURE = 0
LLM_TIMEOUT = int(os.getenv("LLM_TIMEOUT", "20"))
# reasoning_effort для классификации: low = быстро и дёшево, точности хватает
...
# Cooldown между автозапусками улучшения (минуты). По умолчанию — 7 дней.
# Ручной /improve игнорирует cooldown.
AUTO_IMPROVE_COOLDOWN_MINUTES = int(os.getenv("AUTO_IMPROVE_COOLDOWN_MINUTES", "10080"))
